...
def feature_extraction(models, args, data_loader, epoch_num=1):
    X_train = None
    y_train = None
    num_samples = epoch_num*len(data_loader.dataset)*int(args["feature_shape_3"]*args["feature_shape_3"]*args['sample_ratio'])
    cur = 0
    for epoch in range(epoch_num):
        for images, targets in tqdm(data_loader):
            ...
            targets = F.interpolate(targets, size=(args["feature_shape_3"], args["feature_shape_3"]), mode="bicubic")
            ...
            if args['sample_ratio'] != 1:
                n = int(args["feature_shape_3"]*args["feature_shape_3"]*args['sample_ratio'])*len(images)
...
if __name__ == "__main__":
    ...
    for images, targets in tqdm(test_loader):
        ...
        y_pred = y_pred.reshape(-1, 1, args["feature_shape_3"], args["feature_shape_3"])
        prob.append(y_pred)
